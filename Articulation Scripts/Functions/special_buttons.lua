...
function export.cogwheel(ctx, id, size, lock, tooltipText, lockedColor, unlockedColor, background, hover, active, centerColor)
    ...
    reaper.ImGui_PushStyleVar(ctx,reaper.ImGui_StyleVar_FrameRounding(),math.ceil(size/40 * 5))
    ...
    local lineThickness = math.ceil(size/30 * 3)
    ...
    reaper.ImGui_DrawList_AddCircle(draw_list, posX + size * 0.52, posY + size * 0.5, size * 0.14, centerColor,nil, math.ceil(size/30 * 1))
