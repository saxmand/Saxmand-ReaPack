...
local function loop()
    if reloadLocalScript then 
        if reloadLocalScript_count >= 10 then
            readLocalScripts()
    ...
    if visible then
        ...
        if retval or mouseReleased or clearSearch then 
            ...
            if reaper.ImGui_BeginTable(ctx, 'scriptsTbl', #columnsTbl, tableFlags) then
                ...
                --for i, tbl in ipairs(scripts_to_use) do 
                    ...
                    for i, v in ipairs(columnsTbl) do
                        ...
                        elseif v == "Time" then  
                            reaper.ImGui_TableNextColumn(ctx) 
                            local readable_time = tbl.genTime and tbl.genTime ~= " " and os.date("%Y-%m-%d %H:%M:%S", tbl.genTime) or ""
                            ...
                            reaper.ImGui_TableNextColumn(ctx)
                            local name =  tbl.instrumentSettings[v] or "" --database_focus == "Local" and tbl.name or tbl.instrumentSettings.Vendor
            ...
            if focusedScript then 
                ...
                if reaper.ImGui_BeginTable(ctx, 'tablenames', 2, tblFlags, widthOfChild) then
                    ...
                    for column, name in ipairs(previewNames) do 
                        ...
                        reaper.ImGui_TableNextColumn(ctx)
                        reaper.ImGui_TextWrapped(ctx, forceText and forceText or (instrumentSettings and instrumentSettings[name]))
                    end
                    reaper.ImGui_EndTable(ctx)
                end
                --[[
